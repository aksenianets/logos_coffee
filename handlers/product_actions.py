...
async def add_product(update, context):
    ...
    if update.message.from_user.id in ADMIN_IDS or check:
        ...
        logger.warning("User %s, %s tried to add new product", update.message.from_user.username, update.message.from_user.id)
...
async def product_type_button(update, context):
    ...
    logger.info("User %s choose a type of product: %s", update.callback_query.from_user.username, query.data)
    return PRODUCT_NAME
...
async def product_type(update, context):
    ...
    logger.info("User %s choose a type of product: %s", update.message.from_user.username, update.message.text)
    return PRODUCT_NAME

async def product_name(update, context):
    context.user_data["product_name"] = update.message.text
    await update.message.reply_text("Введите опции товра через запятую\nНапример: S, M, L\nЕсли опций нет, то просто напишите: Нет")

    logger.info("User %s choose a name of product: %s", update.message.from_user.username, update.message.text)
...
async def product_options(update, context):
    ...
    if len(product_options) == 1:
        ...
        await update.message.reply_text("Введите цены товара в соответсивии с порядком его опций через запятую")
        
    logger.info("User %s choose option(-s) of product: %s", update.message.from_user.username, product_options)
...
async def product_price(update, context):
    ...
    if len(product_options) != len(product_prices):
        await update.message.reply_text("Количество цен должно соответсвовать количеству опций\nВведите цены ещё раз")
        return PRODUCT_PRICE
    ...
    logger.info("User %s successfully added new product: %s", update.message.from_user.username, context.user_data["product_name"])
...
async def delete_product(update, context):
    ...
    if update.message.from_user.id in ADMIN_IDS or check:
        ...
        await update.message.reply_text("Вы не являетесь админом или сотрудником")
        logger.warning("User %s, %s tried to delete product", update.message.from_user.username, update.message.from_user.id)
        return ConversationHandler.END

async def delete_product_name(update, context):
    context.user_data["product_name"] = update.message.text
    await update.message.reply_text("Введите опции товара через запятую\nНапример: S, M, L\nЕсли опций нет, то просто напишите: Нет")

    logger.info("User %s choose a name of deleting product: %s", update.message.from_user.username, update.message.text)
...
async def change_product_availability(update, context):
    ...
    if update.message.from_user.id in ADMIN_IDS or check:
        ...
        logger.warning("User %s, %s tried to change product availability", update.message.from_user.username, update.message.from_user.id)
        return ConversationHandler.END
...
async def change_product_availability_name(update, context):
    ...
    logger.info("User %s choose a name of changing product availability: %s", update.message.from_user.username, update.message.text)
    ...
    for option in product_options:
        ...
        if check == 0:
            ...
            if option == "нет":
                ...
                await update.message.reply_text(f"Товар {product_name} {option} теперь доступен")

            logger.info("User %s successfully changed product availability: %s %s to 1", update.message.from_user.username, product_name, option)
        elif check == 1:
            change_availability_DB(product_name, option)
            if option == "нет":
                await update.message.reply_text(f"Товар {product_name} теперь недоступен")
            else:
                await update.message.reply_text(f"Товар {product_name} {option} теперь недоступен")

            logger.info("User %s successfully changed product availability: %s %s to 0", update.message.from_user.username, product_name, option)
        else:
            await update.message.reply_text(f"Товара {product_name} {option} не существует")
